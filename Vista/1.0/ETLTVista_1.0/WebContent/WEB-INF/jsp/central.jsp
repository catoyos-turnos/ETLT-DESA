...
<c:set var="titulo" scope="page" value="ETLT -- MIS COSIS"/>
...
<c:set var="fechaIn" value="${sdfIn.format(ts.fecha)}" /><c:set var="fechaEx" value="${sdfEx.format(ts.fecha)}" />
		<tr>
			...
			<div class="ctb_campo_fecha"><a href="${raiz}/res/${residencia.codigo}/dia?fecha=${fechaIn}">${fechaEx}</a></div></th>
			<td><div class="ctb_campo_turno"><a href="${raiz}/turno?cod=${ts.turno.codTurno}">${ts.turno.codTurno}</a></div></td>
			<td>${ts.servicio.hora_pres}</td>
			<td>${ts.servicio.hora_ret}</td>
			<td><div class="ctb_campo_serv_desc"><a href="${raiz}/trab/${trabajador.codigo}/dia?fecha=${fechaIn}">${ts.servicio.descripcion}</a></div></td>
			<td><div class="ctb_bt_soli_cambio"><a href="${raiz}/solicita_cambio_turno?user=${trabajador.codigo}&fecha=${fechaIn}" >SOLICITAR CAMBIO</a></div></td>
		</tr>
</c:forEach>
	</tbody>
	</table>
	<p>&nbsp;</p>
	<p><a href="${raiz}/horario">HORARIO COMPLETO</a></p>
