...
<div class="col-md-4" id="leftCol">
	<div id="panel-side" class="panel panel-default ">
		...
		<div class="panel-heading"><strong><a href="${_raiz}/app/horario">Mi Horario</a></strong></div>
...
<c:set var="fechaIn" value="${sdfIn.format(ts.fecha)}" /><c:set var="fechaEx" value="${sdfEx.format(ts.fecha)}" />
			<tr data-toggle="tooltip" title='${ts.servicio.descripcion}'>
				<th scope="row"<c:if test="${fechaIn eq fechaHoy}"> class="servicio_hoy"</c:if>>
					<div class="ctb_campo_fecha"><a href="${_raiz}/app/res/${residencia.codigo}/dia?fecha=${fechaIn}">${fechaEx}</a></div>
				</th>
				<td><div class="ctb_campo_turno"><a href="${_raiz}/app/turno?cod=${ts.turno.codTurno}">${ts.turno.codTurno}</a></div></td>
